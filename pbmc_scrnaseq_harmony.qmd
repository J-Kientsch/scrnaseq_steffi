...
## 6) On-disk backend (optional)

```{r}
## 6) On-disk backend (BPCells or HDF5Array), but keep an in-memory dgCMatrix copy for SingleR
...
.ensure_harmony_nn_umap <- function(x) {
  ...
  if (!"harmony_nn" %in% names(x@graphs)) {
    x <- FindNeighbors(x, reduction = "harmony", dims = dims_use,
                       k.param = 30, graph.name = "harmony_nn", verbose = FALSE)
  }
  if (!"umap_harmony" %in% names(x@reductions)) {
    x <- RunUMAP(x, reduction = "harmony", dims = dims_use,
                 n.neighbors = 30, min.dist = 0.3, seed.use = 156,
...
cluster_one_resolution <- function(x, r, algo) {
  ...
  x <- FindClusters(x, graph.name = "harmony_nn", resolution = r,
  ...
  p1 <- DimPlot(x, reduction = "umap_harmony", group.by = cl_col,
                label = TRUE, repel = TRUE, raster = TRUE, pt.size = 0.5)
  p2 <- DimPlot(x, reduction = "umap_harmony", group.by = "sample",
                raster = TRUE, pt.size = 0.5)
  p3 <- DimPlot(x, reduction = "umap_harmony", split.by = "sample",
  ...
  markers <- tryCatch(
    FindAllMarkers(x, assay = "SCT", only.pos = TRUE,
  ...
  if (nrow(markers) > 0) {
    ...
    if (length(hm_feats) >= 2) {
      suppressWarnings({
        x <- ScaleData(x, assay = DefaultAssay(x), features = hm_feats, verbose = FALSE, clip.max = 3)
      })
      p_heat <- DoHeatmap(x, features = hm_feats, assay = DefaultAssay(x),
                          slot = "scale.data", raster = TRUE)
      save_both(p_heat, file.path(dirs$plots, sprintf("Heatmap_top10_markers_res%s", r)))

      p_dot <- DotPlot(x, features = hm_feats, group.by = cl_col,
    ...
    for (cl in levels(Idents(x))) {
      genes_cl <- markers %>%
        ...
        dplyr::pull(gene) %>% intersect(rownames(x))
      if (length(genes_cl)) {
        p_vln <- VlnPlot(x, features = genes_cl, group.by = cl_col,
                         pt.size = 0, ncol = min(3, length(genes_cl)))
        save_both(p_vln, file.path(dirs$plots,
                 sprintf("VlnPlot_top_markers_cluster_%s_res%s", cl, r)))
      }
    }
  }

  meta_res <- x@meta.data %>% tibble::rownames_to_column("cell_id") %>%
  ...
  prop_tbl <- meta_res %>%
    ...
    dplyr::ungroup()
  readr::write_csv(prop_tbl, file.path(dirs$meta, sprintf("proportions_by_donor_condition_res%s.csv", r)))
  readr::write_csv(meta_res,  file.path(dirs$meta,  sprintf("metadata_res%s.csv", r)))
