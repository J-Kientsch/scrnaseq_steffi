...
## 7) Doublets (optional, kept commented)
```{r}
# See original script if you want to enable per-sample scDblFinder.
```
...
shared_theme +
theme(plot.title = element_text(face = "bold", size = shared_base + 2))
}, error = function(e) {
...
stopifnot(!is.null(sdev), length(sdev) > 0)
...
last_pt <- dplyr::slice_tail(df_scree, n = 1)
p_scree <- p_scree +
annotate("text",
x = last_pt$PC + 0.8,
...
pal_pos = "#2b83ba", pal_neg = "#de2d26") {
df <- data.frame(gene = rownames(mat),
loading = as.numeric(mat[, pc]),
...
if (!is.null(p_umap_pca_sample)) save_both(p_umap_pca_sample, file.path(plots_dir, "UMAP_preHarmony_by_sample"),   width = 9, height = 9)
if (!is.null(p_umap_pca_donor))  save_both(p_umap_pca_donor,  file.path(plots_dir, "UMAP_preHarmony_by_donor"),    width = 9, height = 9)
if (!is.null(p_umap_pca_cond))   save_both(p_umap_pca_cond,   file.path(plots_dir, "UMAP_preHarmony_by_condition"), width = 9, height = 9)

message("PCA / diagnostics / UMAP (pre-Harmony) completed.")
...
stop_quiet("donor" %in% colnames([obj@meta.data](mailto:obj@meta.data)), "Metadata lacks 'donor' column.")
...
save_both(p_umap_harmony_sample, file.path(plots_dir, "UMAP_harmony_by_sample"),   width = 9, height = 9)
save_both(p_umap_harmony_donor,  file.path(plots_dir, "UMAP_harmony_by_donor"),    width = 9, height = 9)
save_both(p_umap_harmony_cond,   file.path(plots_dir, "UMAP_harmony_by_condition"), width = 9, height = 9)

if (have_lisi) {
emb  <- as.data.frame(Embeddings(obj, "harmony"))
meta <- [obj@meta.data](mailto:obj@meta.data)[, c("donor","condition"), drop = FALSE]
...
save_both(p_umap_clusters, file.path(dirs$plots, "UMAP_clusters_res0.1"),        width = 9, height = 9)
save_both(p_umap_sample,   file.path(dirs$plots, "UMAP_by_sample_res0.1"),       width = 9, height = 9)
save_both(p_umap_split,    file.path(dirs$plots, "UMAP_split_by_sample_res0.1"), width = 9, height = 9)
...
suppressWarnings({
obj <- ScaleData(obj, assay = DefaultAssay(obj),
features = hm_feats, verbose = FALSE, clip.max = 3)
})
p_heat <- DoHeatmap(
...
ggplot2::ggtitle("Heatmap unavailable at res=0.1 (insufficient features)")
}
save_both(p_heat, file.path(dirs$plots, "Heatmap_top10_markers_res0.1"))

# --- NEW: DotPlot of top markers across clusters (res = 0.1) ---

if (length(hm_feats) >= 2) {
p_dot_markers <- DotPlot(
obj, features = hm_feats,
group.by = cl_col,
scale.by = "size", dot.min = 0.1
) +
ggplot2::ggtitle("Top markers dotplot (res=0.1)") +
ggplot2::theme(
axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)
)
save_both(p_dot_markers, file.path(dirs$plots, "DotPlot_top_markers_res0.1"))
}

# --- NEW: Violin plots for top markers per cluster (res = 0.1) ---
...
group.by = cl_col,
pt.size = 0,
ncol = min(3, length(genes_cl))
) +
ggplot2::ggtitle(paste0("Top markers (violin) — cluster ", cl, " (res=0.1)"))

save_both(p_vln_cl, file.path(dirs$plots, paste0("VlnPlot_top_markers_cluster_", cl, "_res0.1")))
}
...
meta_res <- [obj@meta.data](mailto:obj@meta.data) %>%
...
ggplot2::labs(title = "Cluster composition by donor/condition (res=0.1)",
...
ggplot2::labs(title = paste0("Top markers: cluster ", cl, " (res=0.1)"),
x = "Gene", y = "avg_log2FC")
...
save_both(p_umap_clusters, file.path(dirs_02$plots, "UMAP_clusters_res0.2"),        width = 9, height = 9)
save_both(p_umap_sample,   file.path(dirs_02$plots, "UMAP_by_sample_res0.2"),       width = 9, height = 9)
save_both(p_umap_split,    file.path(dirs_02$plots, "UMAP_split_by_sample_res0.2"), width = 9, height = 9)
...
if (length(hm_feats) >= 2) {
suppressWarnings({
obj <- ScaleData(obj, assay = DefaultAssay(obj), features = hm_feats,
verbose = FALSE, clip.max = 3)
})
p_heat <- DoHeatmap(obj, features = hm_feats, assay = DefaultAssay(obj),
slot = "scale.data", raster = TRUE) +
ggplot2::ggtitle("Top markers heatmap (res=0.2)")
} else {
p_heat <- ggplot2::ggplot() +
ggplot2::ggtitle("Heatmap unavailable at res=0.2 (insufficient features)")
}
save_both(p_heat, file.path(dirs_02$plots, "Heatmap_top10_markers_res0.2"))

# --- NEW: DotPlot of top markers across clusters (res = 0.2) ---
...
ggplot2::ggtitle("Top markers dotplot (res=0.2)") +
ggplot2::theme(
axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)
)
save_both(p_dot_markers, file.path(dirs_02$plots, "DotPlot_top_markers_res0.2"))
}

# --- NEW: Violin plots for top markers per cluster (res = 0.2) ---

clusters_02 <- levels(Idents(obj))
for (cl in clusters_02) {
genes_cl <- markers_02 %>%
dplyr::filter(cluster == cl) %>%
dplyr::slice_max(order_by = avg_log2FC, n = 6, with_ties = FALSE) %>%
dplyr::pull(gene) %>%
...
ggplot2::ggtitle(paste0("Top markers (violin) — cluster ", cl, " (res=0.2)"))

save_both(p_vln_cl, file.path(dirs_02$plots, paste0("VlnPlot_top_markers_cluster_", cl, "_res0.2")))
}

if (exists("immune_panel") && length(intersect(immune_panel, rownames(obj))) > 0) {
p_dot <- DotPlot(obj, features = intersect(immune_panel, rownames(obj)),
scale.by = "size", dot.min = 0.1) +
...
save_both(p_umap_clusters, file.path(dirs_05$plots, "UMAP_clusters_res0.5"),        width = 9, height = 9)
save_both(p_umap_sample,   file.path(dirs_05$plots, "UMAP_by_sample_res0.5"),       width = 9, height = 9)
save_both(p_umap_split,    file.path(dirs_05$plots, "UMAP_split_by_sample_res0.5"), width = 9, height = 9)
...
p_heat <- DoHeatmap(obj, features = hm_feats, assay = DefaultAssay(obj),
slot = "scale.data", raster = TRUE) +
ggplot2::ggtitle("Top markers heatmap (res=0.5)")
} else {
p_heat <- ggplot2::ggplot() +
ggplot2::ggtitle("Heatmap unavailable at res=0.5 (insufficient features)")
}
save_both(p_heat, file.path(dirs_05$plots, "Heatmap_top10_markers_res0.5"))

# --- NEW: DotPlot of top markers across clusters (res = 0.5) ---
...
ggplot2::ggtitle("Top markers dotplot (res=0.5)") +
ggplot2::theme(
axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)
)
save_both(p_dot_markers, file.path(dirs_05$plots, "DotPlot_top_markers_res0.5"))
}

# --- NEW: Violin plots for top markers per cluster (res = 0.5) ---

clusters_05 <- levels(Idents(obj))
for (cl in clusters_05) {
genes_cl <- markers_05 %>%
dplyr::filter(cluster == cl) %>%
dplyr::slice_max(order_by = avg_log2FC, n = 6, with_ties = FALSE) %>%
dplyr::pull(gene) %>%
...
ggplot2::ggtitle(paste0("Top markers (violin) — cluster ", cl, " (res=0.5)"))

save_both(p_vln_cl, file.path(dirs_05$plots, paste0("VlnPlot_top_markers_cluster_", cl, "_res0.5")))
}
...
## 18) SingleR — per-cell annotations (res = 0.1)
```{r}
...
readr::write_csv(obj@meta.data %>% tibble::rownames_to_column("cell_id"),
...
save_both(p_umap, file.path(annot_dir, paste0("UMAP_", label_col)), width = 9, height = 9)
}
```

## 19) SingleR — per-cell annotations (res = 0.2)
```{r}
...
## 20) SingleR — per-cell annotations (res = 0.5)
```{r} 
...
## 21) Pseudobulk setup (RNA counts)
```{r}
# Use raw RNA counts for pseudobulk
DefaultAssay(obj) <- "RNA"

# Filtering thresholds for edgeR
pseudo_min_cpm <- 1      # CPM threshold
pseudo_min_smp <- 2      # require expression in at least this many pseudobulk samples

# We'll test IL9 and IL18 *response* differences between healthy vs diseased:
# "NB166","NB171","NB172" = healthy, "AD200","AD201","NB175" = diseased
healthy_donors  <- c("NB166","NB171","NB172")
diseased_donors <- c("AD200","AD201","NB175")

# Re-use saver and dir helper
stopifnot(exists("save_both"), is.function(save_both))
...
## 22) Pseudobulk aggregation (per cluster, per resolution)
```{r}
aggregate_pseudobulk <- function(seurat_obj, res_value, project_dir,
  ...
  cl_vec <- as.character(seurat_obj@meta.data[[cl_col]])
  ...
  bulk_dir <- file.path(dirname(d$meta), "pseudobulk")
  ...
  for (cl in clusters) {
    sel_cells <- rownames(meta)[cl_vec == cl]
    if (length(sel_cells) < min_cells_per_cluster) next

    meta_cl   <- meta[sel_cells, , drop = FALSE]
    counts_cl <- counts[, sel_cells, drop = FALSE]

    samp <- meta_cl$sample
    pb_mat <- rowsum(t(counts_cl), group = samp)
    pb_mat <- t(pb_mat)  # genes × samples
  ...
  message(sprintf(
    "Pseudobulk res=%s: aggregated %d clusters with ≥%d cells (dir: %s)",
    res_value, n_written, min_cells_per_cluster, bulk_dir
  ))
...
## 23) edgeR pseudobulk DE — interaction: healthy vs diseased for IL9/IL18 vs wo
```{r}
suppressPackageStartupMessages({
...
# Run one edgeR model per cluster, and test *interaction*:
# (IL9 - wo in diseased) - (IL9 - wo in healthy)
# (IL18 - wo in diseased) - (IL18 - wo in healthy)
run_interaction_cluster <- function(pb_counts, pb_meta, trt,
                                    healthy_ids, diseased_ids) {

  stopifnot(trt %in% c("IL9","IL18"))

  meta <- pb_meta

  # Derive group from donor
  meta$group <- NA_character_
  meta$group[meta$donor %in% healthy_ids]  <- "healthy"
  meta$group[meta$donor %in% diseased_ids] <- "diseased"
  meta <- meta[!is.na(meta$group), , drop = TRUE]

  # Keep only wo + chosen treatment
  keep <- meta$condition %in% c("wo", trt)
  meta <- meta[keep, , drop = FALSE]
  if (nrow(meta) < 4) return(NULL)

  # Ensure both groups present
  if (length(unique(meta$group)) < 2) return(NULL)

  # Ensure each group has at least one wo and one trt sample
  ...
  # Filter genes: CPM > pseudo_min_cpm in >= pseudo_min_smp samples
  ...
  tt$effect <- paste0(trt, "_interaction_healthy_vs_diseased")

  list(tt = tt, qlf = qlf, y = y, meta = meta, contrast = trt,
       coef_name = coef_name)
}
...
plot_volcano <- function(tbl, title) {
  ...
  ggplot(df, aes(x = logFC, y = neglog10FDR)) +
    ...
    labs(title = title, x = "logFC (interaction)", y = "-log10(FDR)") +
...
for (r in names(pb_all)) {
  cl_list <- pb_all[[r]]
  if (length(cl_list) == 0) {
    message(sprintf("DE: res=%s has no pseudobulk clusters to test.", r))
    next
  }
  ...
  for (cl in names(cl_list)) {
    ...
    for (trt in c("IL9","IL18")) {
      res <- run_interaction_cluster(
        pb_counts = cnt,
        pb_meta   = meta,
        trt       = trt,
        healthy_ids  = healthy_donors,
        diseased_ids = diseased_donors
      )
      if (is.null(res)) next

      out_csv <- file.path(
        de_dir,
        sprintf("DE_edgeR_res%s_cluster_%s_%s_interaction_healthy_vs_diseased.csv",
                r, cl, trt)
      )
      ...
      p <- plot_volcano(
        res$tt,
        sprintf("edgeR interaction: res %s, cluster %s, %s (healthy vs diseased, %s vs wo)",
                r, cl, trt, trt)
      )
      save_both(
        p,
        file.path(de_dir,
                  sprintf("Volcano_res%s_cluster_%s_%s_interaction_healthy_vs_diseased",
                          r, cl, trt))
      )
      ...
      de_summary[[length(de_summary) + 1]] <- data.frame(
        resolution = r,
        cluster    = cl,
        contrast   = paste0(trt, "_interaction"),
        sig_genes  = sig_n,
        stringsAsFactors = FALSE
      )
      n_de_written <- n_de_written + 1L
    }
  }

  message(sprintf(
    "DE: res=%s, wrote %d cluster×treatment interaction tables (dir: %s)",
    r, n_de_written, de_dir
  ))
}

if (length(de_summary)) {
  de_sum_df <- do.call(rbind, de_summary)
  readr::write_csv(de_sum_df, file.path(project_dir, "pseudobulk_DE_summary_counts_interactions.csv"))
} else {
  message("DE: no interaction contrasts passed basic filtering; no DE summary file written.")
